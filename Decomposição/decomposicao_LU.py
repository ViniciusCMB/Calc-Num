...
def decomp_lu(matriz):
    ...
    for i in range(dim):
        for j in range(i, dim):
            # Calcula a soma dos produtos dos elementos de L e U
            soma = sum(L[i][k] * U[k][j] for k in range(i))
            # Preenche a matriz U com os elementos da matriz original após a subtração
        ...
        for j in range(i, dim):
            if i == j:
                ...
                # Preenche a matriz L com os elementos da matriz original após a subtração e a divisão
...
if dimensao == 1:  # Executa a matriz teste
    ...
    for i in range(dimensao):
        ...
        for j in range(dimensao):
            # Obtem os valores dos elementos nas posições i,j
            valor = float(
                input(f"Digite o valor para a posição A[{i+1}],[{j+1}]: "))
            # Preenche as linhas com os valores digitados pelo usuário
            linha.append(valor)
